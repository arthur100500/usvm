...
class JcForbiddenInvocations(val ctx: JcContext, val cp: JcClasspath) {
    ...
    private fun getDescriptions(): Set<JcMethod> {
        return setOf(
            ...
            only("org.springframework.web.servlet.handler.HandlerMappingIntrospector#lambda\$createCacheFilter\$3").methods(all()),
            only("org.springframework.security.web.FilterChainProxy#lambda\$doFilterInternal\$3").methods(all()),
            ...
            only("org.springframework.security.core.context.ThreadLocalSecurityContextHolderStrategy#lambda\$getDeferredContext\$0").methods(all()),
