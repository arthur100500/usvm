...
class JPANativeQueryTranslator(
   ...
   private fun insertSpaces(str: String): String {
    ...
    //fun buildQuery() = insertSpaces(query).split(" ").let { insertAliases(it) }.joinToString(" ")

    fun buildQuery(): String {
        val withSpaces = insertSpaces(query)
        val withAliases = insertAliases(withSpaces.split(" "))
        return withAliases.joinToString(" ").replace("! =", "!=")
    }
}
