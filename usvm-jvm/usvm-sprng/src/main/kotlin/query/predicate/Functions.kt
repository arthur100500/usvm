...
import machine.interpreter.transformers.springjpa.query.Select
...
import machine.interpreter.transformers.springjpa.query.type.ListSqlType
...
class In(val expr: Expression, val list: InValues) : DoublePredicate(expr, list) {

    abstract class InValues: Expression()

    class ListIn(
        val list: Select,
    ) : InValues() {

        override val type = ListSqlType()

        override fun genInst(ctx: MethodCtx) = list.genInstAndWrapToList(ctx)

        override fun getLambdas(info: CommonInfo) = list.getLambdas(info)
    }

    class ParameterIn(
        val param: Parameter
    ) : InValues() {
        override val type = param.type

        override fun genInst(ctx: MethodCtx) = param.genInst(ctx)

        override fun getLambdas(info: CommonInfo) = param.getLambdas(info)
    }

    override fun genInst(ctx: MethodCtx) = with(ctx) {
