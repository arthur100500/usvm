...
open class SqlFunction(val func: Function, val args: List<Expression>) : Expression() {
    ...
    override fun genInst(ctx: MethodCtx) =
        if (func.isAggregator) genAggregatorInst(ctx)
        else genSimpleFuncInst(ctx)

    private fun genAggregatorInst(ctx: MethodCtx) = with(ctx) {
    ...
    private fun genSimpleFuncInst(ctx: MethodCtx) = with(ctx) {
    ...
    enum class Function(
        ...
        COUNT("count", true, { _ -> Primitive.Long() }, true),
        AVG("avg", true, { _ -> Primitive.Double() }, true),
        MIN("min", false, { args -> args.single().type }, true),
        MAX("max", false, { args -> args.single().type }, true),
        SUM("sum", false, { args -> args.single().type.upcastSumRetType() }, true),
        VAR_POP("varPop", true, { _ -> Primitive.Double() }, true),
        VAR_SAMP("varSamp", true, { _ -> Primitive.Double() }, true),
        STDDEV_POP("stddevPop", true, { _ -> Primitive.Double() }, true),
        STDDEV_SAMP("stddevSamp", true, { _ -> Primitive.Double() }, true),
        ANY("any", true, { _ -> Primitive.Bool() }, true),
        EVERY("every", true, { _ -> Primitive.Bool() }, true),

        DATE("date", true, { _ -> Primitive.LocalDate() }, false)
    }
